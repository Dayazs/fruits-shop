			...
			<view class="order-card" v-for="order in orders" :key="order.id">
				...
				<view class="card-footer">
					...
					<view class="footer-actions">
						<button v-if="order.status === 0" class="action-btn cancel"
							@tap="handleCancel(order)">取消订单</button>
						<button v-if="order.status === 0" class="action-btn pay"
							@tap="handlePayOrder(order)">立即支付</button>
					</view>
				</view>
			</view>
		</view>
...
<script setup>
	...
	onLoad((options) => {
		if (options && options.status !== undefined) {
			filterStatus.value = parseInt(options.status)
		}
		const titles = {
			0: '待付款',
			1: '待发货',
		...
		if (filterStatus.value !== null && titles[filterStatus.value]) {
			uni.setNavigationBarTitle({
				title: titles[filterStatus.value]
			})
		}
		fetchOrders()
	})
	...
	const handlePayOrder = async (order) => {
		...
		try {
			...
			const payRes = await orderApi.payOrder({
				order_id: order.id,
				openid
			})
