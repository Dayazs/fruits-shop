		...
		<view v-if="showAddressPicker" class="picker-mask" @tap="showAddressPicker = false">
			<view class="picker-panel" @tap.stop>
				...
				<scroll-view class="picker-list" scroll-y>
					<view class="picker-item" :class="{ active: selectedAddress?.id === addr.id }"
						v-for="addr in addresses" :key="addr.id" @tap="handlePickAddress(addr)">
						<view class="pick-top">
							<text class="pick-name">{{ addr.receiver_name }}</text>
							<text class="pick-mobile">{{ addr.receiver_mobile }}</text>
						</view>
						<text
							class="pick-detail">{{ addr.province }}{{ addr.city }}{{ addr.district }}{{ addr.detail_address }}</text>
	...
	const handlePay = async () => {
		...
		try {
			...
			uni.requestPayment({
				...
				success: async () => {
					try {
						await orderApi.paySuccess(orderId.value)
					} catch (_) {}
					uni.showToast({
						title: '支付成功',
						icon: 'success'
