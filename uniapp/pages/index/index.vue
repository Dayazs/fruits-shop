  ...
  <view class="page">
    ...
    >
      ...
      <view class="goods-grid" v-if="cat.products && cat.products.length > 0">
        <view
          class="goods-card"
          v-for="goods in cat.products"
          :key="goods.id"
          @tap="handleGoodsTap(goods)"
        >
          <image
            :src="IMG_BASE + goods.main_image"
            mode="aspectFill"
            class="goods-img"
          ></image>
          ...
          <view class="goods-price-row">
            <view class="price-left">
              ...
              <text
                v-if="goods.first_sku_original_price"
                class="price-original"
              >¥{{ formatPrice(goods.first_sku_original_price) }}</text>
            </view>
            <view class="cart-btn">
              <image
                src="/static/icons/shopping_trolley_item.svg"
                class="cart-icon"
                mode="aspectFit"
              ></image>
            </view>
...
import { homeApi, IMG_BASE } from '@/utils/api.js'
...
const handleGoodsTap = (goods) => {
  uni.showToast({ title: goods.name, icon: 'none' })
}
</script>
